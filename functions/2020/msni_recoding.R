...
df <- individual_to_HH_numeric(loop, df, "single_famale_hhh", "b2")
df <-
...
df$c3 <-
  case_when(temp$all_child_not_attend[match(df$X_uuid, temp$X_uuid)] == 1 ~ 1,
...
df$f8 <-     case_when(r$not_residing == "yes" ~ 1,
...
df$protection_score <- case_when(
  df$f1 == 1 | df$f2 == 1 ~ 4,
  df$f5 == 1 |
    sum_row(df$f3, df$f4, f6, f7, f8, f9) >= 4 ~ 3,
  sum_row(df$f3, df$f4, f6, f7, f8, f9) == 3 ~ 2,
  sum_row(df$f3, df$f4, f6, f7, f8, f9) >= 1 ~ 1,
  TRUE ~ 0
)
...
df$i3 <- ifelse(r$latrines %in% c("vip_pit", "flush"), 0, 1)
