covid_recoding_2019 <- function(df, loop) {
  ...
  df$c <- case_when(
    df$passport_u18 == "no" |
      df$id_card_u18 == "no" |
      df$citizenship_u18 == "no" | df$birth_cert_u18 == "no" |
      df$marriage_cert_u18 == "non_valid" |
      df$divorce_cert_u18 == "non_valid" ~ 1,
    !(
      df$passport_u18 == "no" |
        df$id_card_u18 == "no" |
        df$citizenship_u18 == "no" | df$birth_cert_u18 == "no" |
        df$marriage_cert_u18 == "non_valid" |
        df$divorce_cert_u18 == "non_valid"
    ) ~ 0
  )
  ...
  df$c18 <-     ifelse(df$child_distress_number < 1 |
  ...
  #please check - needs validation
  df$rent_share <- df$rent_exp/df$tot_expenditure
  df$c28 <-
  ...
  df$c40_iii <- case_when(
    df$aid_not_satisfied.delays == 1 ~ 1,
    (df$aid_received == "yes" &
       df$aid_satisfaction == "yes") |
      df$aid_not_satisfied.delays == 0 ~ 0,
    TRUE ~  NA_real_
  )
  ...
  #please check - requires validation
  df$c47_i <- df$employment_primary_barriers.increased_competition 
  
  #please check - requires validation
  df$c47_ii <- df$employment_primary_barriers.lack_of_connections
  #please check - requires validation
  df$c47_iii <- df$employment_primary_barriers.underqualified_for_jobs 
  #please check - requires validation
  df$c47_iv <- df$employment_primary_barriers.jobs_far 
  
  #please check - requires validation
  df$c49_i <- df$health_barriers.cost
  
  #please check - requires validation
  df$c49_ii <- df$health_barriers.no_medicine
  
  #please check - requires validation
  df$c49_iii <- df$health_barriers.no_offered_treatment
  
  #please check - requires validation
  df$c49_iv <- df$health_barriers.distance_to_treatmentcenter 
  
  #please check - requires validation
  df$c49_v <- df$health_barriers.phc_closed
